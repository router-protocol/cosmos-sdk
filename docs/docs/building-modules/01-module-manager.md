...
* `PreBeginBlock(ctx sdk.Context, req abci.RequestBeginBlock)`: At the beginning of each block, this function is called from [`BaseApp`](../core/00-baseapp.md#beginblock) and, in turn, calls the `PreBeginBlock` function of each modules implementing the `PreBeginBlockAppModule` interface. The function returns a `sdk.ResponsePreBeginBlock{ConsensusParamsChanged: bool}` which indicates whether the consensus parameters are changed or not, if `ConsensusParamsChanged` is true, the caller will refresh the context's consensus parameters, and make the changes visible to followed code. The main use case is for upgrade module to do the state migration before all other code logics.
