...
* `PreBeginBlock(ctx sdk.Context, req abci.RequestBeginBlock)`: At the beginning of each block, this function is called from [`BaseApp`](../core/00-baseapp.md#beginblock) and, in turn, calls the `PreBeginBlock` function. The function returns a `sdk.ResponsePreBeginBlock` indicating whether the ConsensusParamsChanged was changed or not and an error if fails.
